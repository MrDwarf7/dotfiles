...
M.load.trouble = function()
	...
	keymap("n", "<leader>xq", [[<cmd>lua require 'config.ui'.reopen_qflist_by_trouble()<cr>]], opts)
...
M.load.which_key = function()
	...
	which_key.register({
		...
		-- ['<Leader>o'] = { name = '+open/org' }, -- Org not installed
		["<Leader>ol"] = { name = "+open/links" },
		["<Leader>g"] = { name = "+git" },
		["<Leader>x"] = { name = "+quickfixlist" },
		-- ['<Leader>c'] = { name = '+chatgpt', mode = { 'n', 'v' } }, -- Leave off for now
		-- ['<Leader><Tab>'] = { name = '+tab' }, -- Leave off for now
...
 《山鬼》]],
	...
	starter.setup({
		...
		items = {
			{
				{
					action = [[lua require("config.colorscheme").pick_randomly()]],
					name = "pick new theme!",
					section = "Appearance",
				},
				{
					action = [[lua require("config.ui").change_verses()]],
