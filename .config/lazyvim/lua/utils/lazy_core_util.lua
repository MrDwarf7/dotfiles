...
function M.p_pretty_trace(opts)
  opts = opts or {}
...
---@return R
function M.p_try(fn, opts)
  opts = type(opts) == "string" and { msg = opts } or opts or {}
  local msg = opts.msg
  -- error handler
  local error_handler = function(err)
    msg = (msg and (msg .. "\n\n") or "") .. err .. M.p_pretty_trace()
...
---@alias PersFileType "file"|"directory"|"link"
---@param path string
---@param fn fun(path: string, name:string, type:PersFileType):boolean?
function M.p_ls(path, fn)
...
---@param path string
---@param fn fun(path: string, name:string, type:PersFileType)
function M.p_walk(path, fn)
  M.p_ls(path, function(child, name, type)
...
function M.p_walkmods(root, fn, modname)
  modname = modname and (modname:gsub("%.$", "") .. ".") or ""
  M.p_ls(root, function(path, name, type)
    if name == "init.lua" then
      fn(modname:gsub("%.$", ""), path)
    elseif (type == "file" or type == "link") and name:sub(-4) == ".lua" then
      fn(modname .. name:sub(1, -5), path)
    elseif type == "directory" then
      M.p_walkmods(path, fn, modname .. name .. ".")
    end
  end)
end
...
-- function M.p_get_unloaded_rtp(modname, opts)
--   opts = opts or {}
--
--   local topmod = M.p_topmod(modname)
--   if opts.cache and M.unloaded_cache[topmod] then
--     return M.unloaded_cache[topmod], true
--   end
--
--   local norm = M.p_normname(topmod)
--
--   ---@type string[]
--   local rtp = {}
--   local Config = require("lazy.core.config")
...
--         if norm == M.p_normname(plugin.name) then
...
-- end
--
-- function M.p_find_root(modname)
--   local paths, cached = M.p_get_unloaded_rtp(modname, { cache = true })
--
...
--     paths = M.p_get_unloaded_rtp(modname)
...
--   local root, match = M.p_find_root(modname)
--   if not root then
--     return
--   end
--
...
---@alias PersLazyNotifyOpts {lang?:string, title?:string, level?:number, once?:boolean, stacktrace?:boolean, stacklevel?:number}

---@param msg string|string[]
---@param opts? PersLazyNotifyOpts
function M.p_notify(msg, opts)
  ...
  if opts.stacktrace then
    msg = msg .. M.p_pretty_trace({ level = opts.stacklevel or 2 })
  end
...
function M.p_debug(msg, opts)
  ...
  if type(msg) == "string" then
    ...
    opts.lang = "lua"
    M.p_notify(vim.inspect(msg), opts)
  end
end

local function can_merge(v)
  return type(v) == "table" and (vim.tbl_isempty(v) or not M.p_is_list(v))
end
...
function M.p_lazy_require(module)
  local mod = nil
...
---@param key string|string[]
---@param value any
function M.p_key_set(t, key, value)
  local path = type(key) == "table" and key or vim.split(key, ".", true)
  local last = t
  for i = 1, #path - 1 do
    local k = path[i]
